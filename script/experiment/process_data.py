...
def load_scenes():
    try:
        df = pd.read_pickle('../../data/origin/scenes.pkl')
    except:
        df = pd.read_csv('../../data/origin/scenes.csv')
        df['Polygon'] = df['Polygon'].apply(lambda s: eval(s))
        df.to_pickle('../../data/origin/scenes.pkl')
    return df
...
def main():
    ...
    df.to_csv('../../data/origin/expt_scenes.csv', index=None)

    for i in [1000, 1500, 2000, 2500, 5000, 10000, 15000, 20000, 50000]:
        df.head(i).to_csv('../../data/experiment/input/{}.csv'.format(i), index=None)
